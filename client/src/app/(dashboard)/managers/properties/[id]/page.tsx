  ...
  const getCurrentMonthPaymentStatus = (leaseId: number) => {
    ...
    const currentMonthPayment = payments?.find((payment) => {
      payment.leaseId === leaseId &&
        new Date(payment.date).getMonth() === currenDate.getMonth() &&
        new Date(payment.date).getFullYear() === currenDate.getFullYear();
    });
    return currentMonthPayment ? currentMonthPayment.status : "Not Paid";
  };
