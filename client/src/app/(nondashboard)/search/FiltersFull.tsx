...
const FiltersFull = () => {
  ...
  const updateUrl = debounce((newFilters) => {
    ...
    router.push(`${pathName}?${updatedSearchParams.toString()}`); // Update the URL with new search params
  });

  const handleSubmit = () => {
    dispatch(setFilters(localFilters)); // Update the global filters state
  ...
  const handleLocationSearch = async () => {
    try {
      ...
      if (data.features && data.features.length > 0) {
        ...
        setLocalFilters((prev) => ({
          ...prev,
          coordinates: [lng, lat], // Set coordinates based on the first result
        }));
      }
    } catch (err) {
  ...
  const handleReset = () => {
    ...
    dispatch(setFilters(initialState.filters)); // Reset the global filters state
    updateUrl(initialState.filters);
  };

  if (!isFilterFullOpen) {
    return null; // If filters are not open, return null to avoid rendering
  }
